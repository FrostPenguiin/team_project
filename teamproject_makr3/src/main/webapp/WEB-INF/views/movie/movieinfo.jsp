   ...
   <jsp:include page="./theme.jsp" />
  	 <h1 class="title">영화 살펴보기</h1>
        <article>
			...
			<section>
				<div>
					<div class="name"><h3>${data.mv_title }</h3></div>
						<div class="info">
							<ul class="movieinfo">
								<li>감독 ${data.mv_director }</li>
								<li>주연 ${data.mv_actor }</li>
								<li>장르 ${data.mv_genre }</li>
								<li>상영시간 ${data.mv_playtime }</li>
								<li>개봉일 ${data.mv_openDate }</li>
								<li>별점 ${data.mv_point }</li>
							</ul>	
						</div>
							<div class="poster">
								<img img class="moviesrc" src="${path }/resources/img/${data.mv_poster}">
							</div>
								<div class="story">
									${data.mv_story }
								</div>		
			...
			<section>
				<div class="movereview">
					<a href="./movieReview">리뷰 보기</a>
				</div>
				<div class="mb-3 text-end">
					<c:url var="adminUrl" value="/admin" />
						<c:if test="${data.mv_num eq data.mv_num}">
							<button class="btn btn-success" type="button" onclick="location.href='${adminUrl}/updateMovie?id=${data.mv_num}'">수정</button>
			...
			<div class="modal-dialog modal-dialog-centered">
				<div class="modal-content">
					...
					<div class="modal-footer">
						...
						<button type="button" class="btn btn-danger" data-bs-dismiss="modal" onclick="#deleteModal(${data.mv_num});">삭제</button>
					</div>
				</div>
			</div>
		</div>
